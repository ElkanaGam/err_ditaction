...
public class Errors {
	...
	public static void FindErrors(String crntSrc, String crntProd, Map errMap) {
		...
		for (int i = 0 ; i< matchingList.size(); i++) {
			...
			if (s2 == null) {
				...
				//onset part
				PartChecking(s1, s2, "onset",  OnsetOmission, wordConsonants);
				//coda part
				PartChecking(s1, s2, "coda",  CodaOmission, wordConsonants);
				//rhyme part
			}
		}
	}
	...
	static void PartChecking(Syllable src, Syllable prod, String part,  int [][] arr, List <Consonants> wordConsonants) {
		//onset section
		...
		if (src.isCluster) {
			...
			if (part1.size()>0) {
				...
				String cName = a.Name;
				//mark occurence of this consonant
				int [] temp = {1,0};
				...
				if (!a.equals(b)) {
					...
					//segmental error detection
						...
						List <String> tempList = SegmentSubtitution.get(cName);
						if (tempList==null) {
							tempList = new ArrayList<String>();
							
						}
						tempList.add(b.Name);
						SegmentSubtitution.put(cName, tempList);
					}
				}
				else {
					temp[1]++;
